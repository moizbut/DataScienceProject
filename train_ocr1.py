...
LABEL_FILE = os.path.join(DATA_ROOT, "gt_no_spaces.txt")
IMAGE_DIR = os.path.join(DATA_ROOT, "test_processed")
VOCAB_FILE = os.path.join(DATA_ROOT, "/Users/moizmac/DataScienceProject/data/UTRSet-Real/UrduGlyphs.txt")

# Hyperparameters
IMG_HEIGHT = 32  # Reduced from 64
...
class UrduOCRDataset(Dataset):
    def __init__(self, label_files, image_dirs, char_to_idx, transform=None):
        self.image_dirs = image_dirs
        self.transform = transform
        self.char_to_idx = char_to_idx
        self.data = []
        for label_file, image_dir in zip(label_files, image_dirs):
    ...
    def __getitem__(self, idx):
        ...
        label_encoded = [self.char_to_idx[char] for char in label if char in self.char_to_idx]
...
class UrduOCRModel(nn.Module):
    def __init__(self, num_chars, hidden_size=256, num_layers=2, dropout=0.2):
        ...
        self.fc = nn.Linear(32 * 32, hidden_size)  # Adjusted for IMG_HEIGHT=32, channels=32
        self.rnn = nn.LSTM(
    ...
    def forward(self, x):
        batch_size = x.size(0)
        c = self.cnn(x)
        c = c.permute(0, 3, 1, 2)  # (batch_size, IMG_WIDTH, IMG_HEIGHT, channels)
        c = c.reshape(batch_size, c.size(1), -1)  # (batch_size, IMG_WIDTH, IMG_HEIGHT * channels)
        c = self.fc(c)
        r, _ = self.rnn(c)
        out = self.fc_out(r)
        return out
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for batch_idx, (images, labels, label_lengths) in enumerate(val_loader):
                ...
                if batch_idx == 0:  # Log for first batch only
...
def main():
    ...
    char_to_idx, idx_to_char = create_vocab(label_files)
    num_chars = len(char_to_idx)
    print(f"Vocabulary size: {num_chars}")
    
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5,), (0.5,))  # Remove this if normalization is not needed
    ])
    
    dataset = UrduOCRDataset(label_files, image_dirs, char_to_idx, transform)
    print(f"Dataset size: {len(dataset)} samples")
    if len(dataset) == 0:
        raise ValueError("No valid data loaded. Check image paths and gt.txt format.")
